            ...
            <div style="display: flex;flex-direction: row;justify-content: space-between;align-items: center;">
                ...
                <div class="card-tools">
                    <div class="align-left">
                        ...
                        <a type="button" class="btn btn-default btn-sm" onclick="createUnitModal()">Add Unit</a>
                        <button type="button" id="delete_unit_btn" class="btn btn-default btn-sm" onclick="deleteUnit()">Delete selected unit</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <table id="employee_tbl" class="table table-bordered">
                ...
                <tbody>
                    ...
                    foreach ($prodUnits as $unit) {
                        $status = ($unit['status'] == 0) ? '<span style="color: green;">Active</span>' : '<span style="color: red;">Inactive</span>';
                    ?>
                        <tr>
                            <td class="no-sort text-center"><input type='checkbox' name='checkbox' value='<?= $unit['id'] ?>'></td>
                            <td class="no-sort text-center">
                                <a style="color: #605e5e;" onclick="editProductUnit('<?= $unit['id'] ?>')"><i class="far fa-edit"></i></a>
                            </td>
                            <td><?= (!empty($unit['product_category'][0]['name'])) ? $unit['product_category'][0]['name'] : $unit['product_category']['name']; ?></td>
                            <td><?= $unit['name'] ?></td>
...
<?php include_once __DIR__ . '/create_unit_modal.php'; ?>
<?php include_once __DIR__ . '/edit_unit_modal.php'; ?>
